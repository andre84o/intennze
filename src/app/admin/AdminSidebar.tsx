...
function filterMenuByRole(items: MenuItem[], role: "admin" | "staff"): MenuItem[] {
  if (role === "admin") return items;
  return items.filter((item) => STAFF_VISIBLE_HREFS.has(item.href));
}

export default function AdminSidebar() {
  const { sidebarState, setSidebarState, openSidebar, role } = useAdmin();
  const pathname = usePathname();
  const asideRef = useRef<HTMLElement>(null);
  const [menuItems, setMenuItems] = useState<MenuItem[]>(() =>
    filterMenuByRole(defaultMenuItems, role)
  );
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.sidebarOrder && Array.isArray(data.sidebarOrder)) {
            ...
            setMenuItems(filterMenuByRole([...orderedItems, ...newItems], role));
          }
        }
      } catch (error) {
