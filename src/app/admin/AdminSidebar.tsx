  ...
  return (
    <aside
      className={`fixed top-0 left-0 h-full bg-slate-900/95 backdrop-blur-xl border-r border-slate-800 z-40 transition-all duration-300 ease-in-out ${
        isOpen ? "w-64" : isCollapsed ? "w-20" : "w-0 -translate-x-full"
      }`}
    >
      <div className={`${isHidden ? "opacity-0" : "opacity-100"} transition-opacity duration-200`}>
        ...
        <div className="h-16 flex items-center px-6 border-b border-slate-800">
          <Link href="/admin" className="flex items-center gap-3">
            ...
            <span
              className={`font-bold text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-purple-400 transition-opacity duration-300 ${
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-slate-800">
          ...
          >
            <svg
              className="w-5 h-5 flex-shrink-0 text-slate-400 group-hover:text-cyan-400 transition-colors"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
              strokeWidth="1.5"
