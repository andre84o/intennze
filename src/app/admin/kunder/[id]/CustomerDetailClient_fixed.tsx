      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          <select
            ...
            disabled={saving}
            className={`px-3 py-2 rounded-lg text-sm font-medium border ${statusStyles[customer.status]} focus:outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer`}
          >
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
            ...
            <div className="space-y-4">
              {customer.email && (
                <div>
                  <label className="text-xs text-gray-500 uppercase tracking-wider">E-post</label>
                  <a href={`mailto:${customer.email}`} className="block text-blue-600 hover:text-blue-700 hover:underline">
                    {customer.email}
                  </a>
                </div>
              )}
              {customer.phone && (
                <div>
                  ...
                  <a href={`tel:${customer.phone}`} className="block text-blue-600 hover:text-blue-700 hover:underline">
                    {customer.phone}
                  </a>
                </div>
              )}
              {(customer.address || customer.city) && (
                <div>
                  <label className="text-xs text-gray-500 uppercase tracking-wider">Adress</label>
              ...
              {customer.org_number && (
                <div>
                  <label className="text-xs text-gray-500 uppercase tracking-wider">Org.nummer</label>
            ...
            <div className="mt-6 pt-6 border-t border-gray-100 flex gap-2">
              {customer.phone && (
                <a
                  ...
                  className="flex-1 px-3 py-2 bg-green-50 hover:bg-green-100 text-green-700 border border-green-200 rounded-lg text-center text-sm transition-colors font-medium"
                >
                  Ring
                </a>
              )}
              {customer.email && (
                <a
                  ...
                  className="flex-1 px-3 py-2 bg-blue-50 hover:bg-blue-100 text-blue-700 border border-blue-200 rounded-lg text-center text-sm transition-colors font-medium"
                >
                  Maila
                </a>
              )}
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="bg-gray-50 rounded-lg p-3 text-center border border-gray-100">
                ...
                <p className="text-xs text-gray-500">Interaktioner</p>
              </div>
            </div>
          </div>
        </div>
            ...
            {showInteractionForm && (
              <form onSubmit={handleAddInteraction} className="mb-6 p-4 bg-gray-50 border border-gray-200 rounded-lg">
                ...
                <div className="flex justify-end gap-2">
                  ...
                  <button
                    type="submit"
                    className="px-4 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors font-medium shadow-sm"
                  >
                    Spara
                  </button>
                </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
              ...
              <div className="space-y-2">
                {reminders.map((reminder) => (
                  ...
                  >
                    <div>
                      <p className={`font-medium ${reminder.is_completed ? "text-gray-500 line-through" : "text-gray-900"}`}>
                        {reminder.title}
                      </p>
